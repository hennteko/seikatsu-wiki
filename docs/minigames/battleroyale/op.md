...
| `shrinkTime` | 60 | ワールドボーダーが縮小する間隔（秒） |
...
| `itemPool` | アイテムIDのリスト | チェストにランダムで入るアイテム（MinecraftのアイテムID） |
...
!!! note "アイテムプールの動作"
    各チェストには、開始時に `itemPool` からランダムで 3〜9 個のアイテムが補充されます。`itemPool` が空の場合、チェストにはアイテムが入りません。アイテムは1個ずつスタックで配置されます。
