...
色鬼（改造版）の導入・スポーン/ステージ/フィールド設定・看板・config（パレット・難易度カーブ）・権限をまとめます。
...
| api-version | 26.1.2 |
| 作者 | henry |
| メインコマンド | `/colortag`（エイリアス `/ct`） |
| 依存プラグイン | なし |
| 設定ファイル | `plugins/ColorTag/config.yml`（設定・座標・看板をすべて格納） |

!!! info "ゲームの仕組み（色当て × 床消滅 × 残機制）"
    鬼はホットバーの染料を右クリックして「安全色」を選びます。猶予（`grace`）のあと、安全色以外のコンクリート床が一斉に空気へ置き換わり、乗れていない逃走者が落下します。落下するごとに残機が1減り、初期スポーンへ復帰。残機0で観戦モードになります。床は猶予＋復活間隔（`recover`）後に元のブロックへ戻ります。経過時間で猶予・復活間隔・安全色数が変わり、終盤はフェイント（予告色から別色へ切替）が発動します。
...
参加・離脱・鬼立候補・開始の **4種** の看板を登録できます。**プレイヤー操作はすべて看板経由**（コマンドでの join/leave/start は無し）なので、4種とも設置してください。看板を見ながら（6ブロック以内）以下を実行します。
...
クリック判定は保存した看板位置との一致で行います（手書きの看板では機能しません）。
...
これらは即 `config.yml` に保存されます（パレット・難易度カーブの編集は config.yml を直接編集します）。
...
| キー | 既定値 | 説明 |
|---|---|---|
| `settings.game-time` | 180 | 制限時間（秒）。経過で生存者の勝利（既定3分） |
| `settings.default-zanki` | 2 | 残機。落下するごとに -1、0で観戦モード |
...
| `grace` | 予告から床消滅までの猶予（秒） |
| `recover` | 床が復活するまでの間隔（秒） |
| `safe-colors` | 安全な色の数（鬼が選んだ色＋追加色） |
| `feint` | フェイント（予告後に色を切替）の許可（`true`/`false`） |

既定の難易度カーブは次のとおりです。

| 経過 | grace | recover | safe-colors | feint |
|---|---|---|---|---|
| 0秒〜 | 4.0 | 5.0 | 2 | false |
| 60秒〜 | 2.5 | 3.0 | 1 | false |
| 120秒〜 | 1.2 | 1.5 | 1 | true |
...
!!! note "難易度・パレットの編集は config.yml を直接編集"
    `palette` と `difficulty` を変更するコマンドはありません。`config.yml` を直接編集してください。`reload` コマンドが無いため、**変更の反映にはサーバー再起動** が必要です（`setzanki` / `settime` はコマンドで即反映されます）。
...
| 権限 | 既定 | 用途 |
|---|---|---|
| `colortag.admin` | OP | `/colortag` コマンド全体（setsign・setlobby・setspawn系・setstage・setfield・setzanki・settime・status） |

!!! note "プレイヤー操作は看板のみ・権限不要"
    プレイヤー向けのコマンド（join / leave / start 等）は **存在しません**。参加・離脱・鬼立候補・開始は **すべて看板クリック** で行い、権限は不要です（看板はイベント経由で全員が操作可）。`/colortag` コマンドは設定・管理専用で、`colortag.admin`（既定OP）が必要です。`status` も `colortag.admin` が必要な点に注意してください。

## 管理コマンド

すべて `colortag.admin`（既定OP）が必要です。`start` / `stop` / `reload` コマンドはありません（開始は開始看板、強制終了はサーバーのプラグイン無効化時に自動終了、設定の再読み込みは再起動）。
...
!!! tip "TAB補完に対応"
    `/colortag` のサブコマンド、`setsign` の種別（join/leave/oni/start）、`setfield` の番号（1/2）はTAB補完できます。
...
??? failure "config.yml（パレット・難易度）を編集したのに反映されない"
    `reload` コマンドが無いため、`palette` / `difficulty` の変更は **サーバー再起動** で反映されます。`setzanki` / `settime` はコマンド実行で即反映されます。
