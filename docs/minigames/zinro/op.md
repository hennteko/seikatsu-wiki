...
プレイヤーが右クリックで参加・退出できる看板を設置します。看板の1行目・2行目に以下を記入します。
...
!!! note "プレイヤー向けの権限"
    `/zinro shop` などプレイヤー向け操作に専用の権限ノードはありません。参加・退出はゲート看板で行うため、一般プレイヤーは権限なしで遊べます。

## 管理コマンド

すべて `zinro.admin` 権限が必要です（`/zinro shop` のみ全員利用可）。
