...
!!! success "ゲート看板は複数設置できます（v更新）"
    参加・退出・ショップ・開始の各ゲート看板を **複数拠点に設置** できるようになりました。`/zinro setsign <join|leave|shop|start>` は上書きではなく **追記** され、`/zinro setsign delete` は視線先の1枚だけ解除します。人数表示（4行目）も全枚数が同時に更新されます。**旧形式（`signs.<種別>.world/x/y/z` の単一座標）は起動時に自動で `signs.<種別>`（`"ワールド名,x,y,z"` のリスト）へ移行されます。設定のやり直しは不要**です（占い看板の `boards.board_1〜20` は従来どおり番号ごとの座標です）。
