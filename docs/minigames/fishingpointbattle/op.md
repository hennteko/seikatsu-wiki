...
```text title="視線先の看板を開始看板として登録（識別子は任意・省略時は main）"
/fpb setsign start <識別子>
```

```text title="視線先の看板の登録を解除（看板を見ながら実行・1枚ずつ）"
/fpb setsign delete
```

!!! success "看板は複数設置できます（v更新）"
    `setsign` は **join / leave / start の3種類** に対応し、いずれも **複数設置** できるようになりました。参加（`join`）・退出（`leave`）は何枚でも設置でき、開始（`start`）は **識別子ごと**（`/fpb setsign start <識別子>`、省略時は `main`）に、**同じ識別子でも何枚でも**設置できます。登録は上書きではなく追記され、`/fpb setsign delete` は視線先の1枚だけ解除します。開始看板を右クリックするだけでゲームを開始できます。**旧形式（単一座標）は起動時に自動でリストへ移行されます。設定のやり直しは不要**です。識別子は半角英数字・ハイフン・アンダースコアのみ使用できます。
