...
!!! success "v3.1 アップデート概要"
    v3.1 では **セットアップコマンドが大幅に簡略化** されました。これまでロビー（lobby）とエリア（area）は別IDで管理し `setup link` で紐付ける必要がありましたが、v3.1 からは **「ゲーム名」1つに統合** され、`/dorokei setup <ゲーム名> lobby` / `pos1` / `pos2` / `jail` を順番に実行するだけで設定が完結します（`setup link` は廃止）。`/dorokei setup <ゲーム名> delete` でゲーム単位の削除も可能になりました。
