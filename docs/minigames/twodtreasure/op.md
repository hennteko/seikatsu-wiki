...
| `y-min` | 1 | 会場の最下層。**この高さに床（ベッドロック）が敷かれます** |
| `y-max` | 320 | 会場の最大Y座標 |

!!! note "地形生成の仕様（更新）"
    マップ生成は会場の **外周のみをバリアブロックで囲い**、内部を `y-min`（床のベッドロック）〜`y-ground-level`（地表）の範囲で土・石・鉱石（ダイヤ/金/鉄/銅/石炭）に上書きします。地表より上は空気にします。会場全体を大きく掘り抜く方式ではないため、**会場サイズを変えて再設定した場合は旧会場のブロックが残ることがあります**（専用ワールドでの新規会場を推奨）。`y-min` は既定で 1 のため、床はY=1に敷かれます。
...
!!! tip "ゲームエリアはX軸の幅で決まります"
    マップ生成はエリアの最小X〜最大X、固定Z（`fixed-z`）、Y軸（`y-min`〜`y-ground-level`）の範囲で行われます。プレイ面は `fixed-z` の1マスに固定され、`setfield` の **X座標の範囲** がプレイエリアになります（pos1/pos2 の Y・Z は使われません）。**X幅は最低でも5ブロック以上** 離して設定してください（スポーン地点・宝箱の配置計算に余裕が必要で、極端に狭いと正しく生成されません）。
