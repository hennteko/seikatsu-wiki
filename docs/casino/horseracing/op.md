...
次の3つは権限不要で **一般プレイヤーも実行可能** です（看板の代わりに使えます）。

| コマンド | 説明 |
|---|---|
| `/horseracing join` | ロビー地点へテレポート（`lobby` 看板と同等） |
| `/horseracing leave` | スポーン地点へテレポート（`leave` 看板と同等） |
| `/horseracing status` | レースの設定状況（地点・看板）を表示（閲覧専用） |
