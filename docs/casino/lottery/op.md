...
一般プレイヤーは `[宝くじ]` 看板の右クリックで抽選券を購入します。看板を設置し、看板を見ながら（6ブロック以内）以下のコマンドを実行すると自動で整形されます。

```text title="購入看板を登録（例: ore を1枚。種類: ore / food / function / build / other）"
/kuzi setsign ore 1
```

```text title="購入看板を登録（種類・枚数を指定。枚数省略時は1枚）"
/kuzi setsign <種類> [枚数]
```
...
!!! note "手書き設置もサポート"
    看板の1行目に `[宝くじ]`（または `[Takarakuzi]`）と手書きしても登録できますが、種類・枚数を正しく書く必要があるためコマンドでの設置を推奨します。
...
| `/kuzi setsign <種類> [枚数]` | 視線先の看板を `[宝くじ]` 購入看板として登録する |
...
!!! tip "TAB 補完"
    OP または `kuzi.reload` 権限保持者が `/kuzi` の第1引数で TAB を押すと、補完候補に `reload` が表示されます。`kuzi.admin` 保持者には `setsign` も表示されます。
...
| `kuzi.admin` | OP | `/kuzi` コマンド（購入・`setsign`）の実行可否 |
